...
export const AddModal = Vue.extend({
  ...
  methods: {
    ...
    validate () {
      ...
      if (!this.form.select) {
        this.validators.select = false
      }

      if (!this.form.text) {
        this.validators.text = false
      }

      if (!this.form.multiline) {
        this.validators.multiline = false
      }

      if (!this.form.radio) {
        this.validators.radio = false
      }
      ...
      if (!this.form.check || this.form.check.length === 0) {
        this.validators.check = false
      }
    }
  }
})
